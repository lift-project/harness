...
void run_harness(std::vector<std::shared_ptr<Run>> &all_run, const size_t N,
                 const std::string &mat_file, const std::string &vecX_file,
                 const std::string &vecY_file, const std::string &gold_file,
                 const bool force, const bool transposeIn, const bool threaded,
                 const bool binary) {
  using namespace std;
  ...
  if (File::is_file_exist(gold_file) && File::is_file_exist(mat_file) &&
      File::is_file_exist(vecX_file) && File::is_file_exist(vecY_file) &&
      !force) {
  ...
  cl::Buffer vecX_dev =
      OpenCL::alloc(CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, N * sizeof(float),
                    static_cast<void *>(vecX.data()));
  cl::Buffer vecY_dev =
      OpenCL::alloc(CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, N * sizeof(float),
                    static_cast<void *>(vecY.data()));
