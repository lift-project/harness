...
void set_kernel_args(const shared_ptr<Run> run, const cl::Buffer &image_dev,
                     ...
                     const cl::Buffer &ds_dev, const cl::Buffer &de_dev,
                     const cl::Buffer &dw_dev, const cl::Buffer &output_dev,
...
void compute_gold(const size_t M, const size_t N, Matrix<float> &image,
                  Matrix<float> &coeffs, Matrix<float> &dDN, Matrix<float> &dDS,
                  Matrix<float> &dDE, Matrix<float> &dDW, Matrix<float> &gold,
  ...
  File::load_input_debug(gold, "/home/bastian/development/exploration/datasets/"
                               "srad_data/finalImage.txt");
  File::load_input_debug(image, "/home/bastian/development/exploration/"
                                "datasets/srad_data/imagebefore.txt");
  File::load_input_debug(
      coeffs,
      "/home/bastian/development/exploration/datasets/srad_data/coeffs.txt");
  File::load_input_debug(
      dDN, "/home/bastian/development/exploration/datasets/srad_data/dDN.txt");
  File::load_input_debug(
      dDS, "/home/bastian/development/exploration/datasets/srad_data/dDS.txt");
  File::load_input_debug(
      dDE, "/home/bastian/development/exploration/datasets/srad_data/dDE.txt");
  File::load_input_debug(
      dDW, "/home/bastian/development/exploration/datasets/srad_data/dDW.txt");
...
void run_harness(std::vector<std::shared_ptr<Run>> &all_run, const size_t M,
                 const size_t N, const std::string &image_file,
  ...
  if (File::is_file_exist(gold_file) && File::is_file_exist(image_file) &&
    ...
    compute_gold(M, N, image, coeffs, dDN, dDS, dDE, dDW, gold, image_file,
                 coeffs_file, dDN_file, dDS_file, dDE_file, dDW_file,
                 gold_file);
  }
  ...
  cl::Buffer coeffs_dev =
      OpenCL::alloc(CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, buf_size,
                    static_cast<void *>(coeffs.data()));
  cl::Buffer dDN_dev = OpenCL::alloc(CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR,
                                     buf_size, static_cast<void *>(dDN.data()));
  cl::Buffer dDS_dev = OpenCL::alloc(CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR,
                                     buf_size, static_cast<void *>(dDS.data()));
  cl::Buffer dDE_dev = OpenCL::alloc(CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR,
                                     buf_size, static_cast<void *>(dDE.data()));
  cl::Buffer dDW_dev = OpenCL::alloc(CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR,
                                     buf_size, static_cast<void *>(dDW.data()));
