...
template <typename T>
void execute_run(const size_t &M, const size_t &N,
		 std::function<bool(const std::vector<T> &)> validate, const cl::Buffer &matA_dev,
		 const cl::Buffer &matB_dev, const cl::Buffer &output_dev,
 ...
 */
template <typename T>
void run_harness(std::vector<std::shared_ptr<Run>> &all_run, const size_t M, const size_t K,
		 const size_t N, const std::string &matA_file, const std::string &matB_file,
		 const std::string &gold_file, const bool force, const bool transposeA,
		 const bool transposeB, const bool transposeOut, const bool threaded,
		 const bool binary) {
	using namespace std;
	...
	if (File::is_file_exist(gold_file) && File::is_file_exist(matA_file) &&
	    File::is_file_exist(matB_file) && !force) {
		...
		if (M % n_threads != 0) n_threads = 16;
		assert(M % n_threads == 0);
		auto chunk = M / n_threads;
		for (unsigned tid = 0; tid < n_threads; tid++)
			threads.push_back(
			    std::thread([=] { mmult(tid * chunk, (tid + 1) * chunk); }));
		for (auto &t : threads)
			t.join();

		std::cout << "Transposing matrices..." << std::endl;

		if (transposeA) transpose(matA, M, K);

		if (transposeB) transpose(matB, K, N);

		if (transposeOut) transpose(gold, M, N);
	...
	cl::Buffer matA_dev = OpenCL::alloc(CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, buf_size_A,
					    static_cast<void *>(matA.data()));
	cl::Buffer matB_dev = OpenCL::alloc(CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, buf_size_B,
					    static_cast<void *>(matB.data()));
		...
		auto execute_thread = std::thread([&] {
			...
			while (!done) {
				...
				while (!ready_queue.empty()) {
					...
					execute_run(M, N, validate, matA_dev, matB_dev, output_dev,
						    r);
				}
			}
		});
