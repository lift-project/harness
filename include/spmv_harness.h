...
void run_harness(std::vector<std::shared_ptr<Run>> &all_run,
                 ...
                 const size_t v_Length_cl, const OpenCLSparseMatrix<T> matrix,
                 const Kernel<T> hl_kernel, const bool threaded,
                 const bool binary) {

  // compile our kernel
  auto kernel = OpenCL::compileSource(hl_kernel.getSource());

  if (kernel.isCompiled()) {

    // start trying to run it.
  } else {
  ...
  //         set_kernel_args(r, roomtminus1_dev, roomt_dev, output_dev, M, N,
  //         O);
