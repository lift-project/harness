 ...
 */
template <typename T>
void run_harness(std::vector<std::shared_ptr<Run>> &all_run, const size_t num_points,
		 const size_t num_clusters, const size_t num_features,
		 const std::string &points_file, const std::string &clusters_file,
		 const std::string &gold_file, const bool force, const bool threaded,
		 const bool binary, const bool transposePoints) {
	using namespace std;
	...
	if (File::is_file_exist(gold_file) && File::is_file_exist(points_file) &&
	    File::is_file_exist(clusters_file) && !force) {
		...
		for (auto point_id = 0; point_id < num_points; point_id++) {
			{
				...
				for (int i = 0; i < num_clusters; i++) {
					...
					for (int l = 0; l < num_features; l++) {
						ans += (points[l + num_features * point_id] -
							clusters[i * num_features + l]) *
						       (points[l + num_features * point_id] -
							clusters[i * num_features + l]);
					}
		...
		if (transposePoints) transpose(points, num_points, num_features);
	...
	cl::Buffer clusters_dev =
	    OpenCL::alloc(CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, buf_size_clusters,
			  static_cast<void *>(clusters.data()));
		...
		auto execute_thread = std::thread([&] {
			...
			while (!done) {
				...
				while (!ready_queue.empty()) {
					...
					OpenCL::executeRun<int>(*r, output_dev, num_points,
								validate);
				}
			}
		});
