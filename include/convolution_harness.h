...
#ifndef EXECUTOR_CONVOLUTION_HARNESS_H
#define EXECUTOR_CONVOLUTION_HARNESS_H
...
void run_harness(std::vector<std::shared_ptr<Run>> &all_run, const size_t N,
  ...
  std::vector<float> weights = {0.5f, 1.0f, 0.5f};
  ...
  if (File::is_file_exist(gold_file) && File::is_file_exist(grid_file)) {
    ...
    for (unsigned i = 0; i < N; i++) {
      ...
      int right = i + 1;
      left = max(0, left);
      right = min(static_cast<int>(N - 1), right);

      gold[i] = grid[left] * weights[0] + grid[i] * weights[1] +
                grid[right] * weights[2];
    }
    ...
    auto execute_thread = std::thread([&] {
      ...
      while (!done) {
        ...
        while (!ready_queue.empty()) {
          ...
          r->getKernel().setArg(3, static_cast<int>(N));
...
#endif // EXECUTOR_CONVOLUTION_HARNESS_H
