 ...
 */
template <typename T>
void run_harness(std::vector<std::shared_ptr<Run>> &all_run, const size_t size,
                 float espSqr, float deltaT, const std::string &positions_file,
                 const std::string &velocities_file,
                 const std::string &gold_file, const bool force,
  ...
  if (File::is_file_exist(gold_file) && File::is_file_exist(positions_file) &&
      File::is_file_exist(velocities_file) && !force) {
    ...
    for (cl_uint i = 0; i < size; ++i) {
      ...
      for (int k = 0; k < 3; ++k) {
        gold[myIndex * 2 + k] = positions[myIndex + k] +
                                velocities[myIndex + k] * deltaT +
  ...
  cl::Buffer positions_dev =
      OpenCL::alloc(CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, buf_size_positions,
                    static_cast<void *>(positions.data()));
  cl::Buffer velocities_dev = OpenCL::alloc(
      CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, buf_size_velocities,
      static_cast<void *>(velocities.data()));
