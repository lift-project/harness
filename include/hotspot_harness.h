...
float rodiniaUserFun(float power, float top, float bottom, float left,
                     float right, float center) {
  {
    float step_div_cap = 1.365333e+00;
    return center +
           step_div_cap * (power + 0.1f * (bottom + top - 2 * center) +
                           0.1 * (left + right - 2 * center) +
                           4.882813e-05 * (80.0f - center));
  }
}
...
void compute_gold(const size_t M, const size_t N, Matrix<float> &temp,
                  Matrix<float> &power, Matrix<float> &gold,
                  const std::string &temp_file, const std::string &power_file,
                  const std::string &gold_file) {

  // File::load_input_debug(gold,
  // "/home/bastian/development/exploration/executor/datasets/"
  //			     "rodiniaData/lift_hotspot_gold_8192_nonBinary");
  File::load_input_debug(
      temp, "/home/bastian/development/exploration/executor/datasets/"
            "rodiniaData/lift_hotspot_temp_8192_nonBinary");
  File::load_input_debug(
      power, "/home/bastian/development/exploration/executor/datasets/"
  ...
  for (int v_gl_id_6 = 0; v_gl_id_6 < 8192; v_gl_id_6++) {
          for (int v_gl_id_7 = 0; v_gl_id_7 < 8192; v_gl_id_7++) {
                  v__5 = rodiniaUserFun(
                      ...
                      temp[((v_M_3 * (((-1 + v_gl_id_6 + (v_gl_id_7 / v_M_3)) >=
  0)
                                          ? (((-1 + v_gl_id_6 + (v_gl_id_7 /
  v_M_3)) < v_N_4)
                                                 ? (-1 + v_gl_id_6 + (v_gl_id_7
  / v_M_3))
                                                 : (-1 + v_N_4))
                                          : 0)) +
                            (((v_gl_id_7 % v_M_3) >= 0)
                                 ? (((v_gl_id_7 % v_M_3) < v_M_3) ? (v_gl_id_7 %
  v_M_3)
                                                                  : (-1 +
  v_M_3))
                                 : 0))],
...
void run_harness(std::vector<std::shared_ptr<Run>> &all_run, const size_t M,
                 const size_t N, const std::string &temp_file,
