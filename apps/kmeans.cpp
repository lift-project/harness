...
int main(int argc, char *argv[]) {
  ...
  // K-Means options
  auto opt_num_points = op.addOption<std::size_t>(
      {'s', "points", "Matrix size (default 1024).", 1024});
  auto opt_num_clusters = op.addOption<std::size_t>(
      {'c', "clusters", "Number of clusters (default 5).", 5});
  auto opt_num_features = op.addOption<std::size_t>(
      {'f', "features", "Number of features (default 34).", 34});

  auto opt_dont_transpose_points = op.addOption<bool>(
      {'t', "no-transpose", "Don't points before passing to kernel", false});
  ...
  auto all_run = Csv::init(
      [&](const std::vector<std::string> &values) -> std::shared_ptr<Run> {
        return (opt_double->get()
                    ...
                    std::shared_ptr<Run>(new KMeansRun<double>(
                        values, num_points, num_clusters, num_features, local_0,
                        local_1, local_2))
                    : std::shared_ptr<Run>(new KMeansRun<float>(
                          values, num_points, num_clusters, num_features,
  ...
  // run the harness
  if (opt_double->get())
    run_harness<double>(all_run, num_points, num_clusters, num_features,
                        points_file, clusters_file, gold_file, opt_force->get(),
                        opt_threaded->get(), opt_binary->get(),
                        transpose_points);
  else
    run_harness<float>(all_run, num_points, num_clusters, num_features,
                       points_file, clusters_file, gold_file, opt_force->get(),
                       opt_threaded->get(), opt_binary->get(),
                       transpose_points);
}
