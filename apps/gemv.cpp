...
int main(int argc, char *argv[]) {
  ...
  auto all_run = Csv::init(
      [&](const std::vector<std::string> &values) -> std::shared_ptr<Run> {
        return (opt_double->get()
                    ? std::shared_ptr<Run>(new GEMVRun<double>(values, N))
                    : std::shared_ptr<Run>(new GEMVRun<float>(values, N)));
      });
  if (all_run.size() == 0)
    return 0;
  ...
  // run the harness
  run_harness(all_run, N, mat_file, vecX_file, vecY_file, gold_file,
              opt_force->get(), opt_transpose->get(), opt_threaded->get(),
              opt_binary->get());
}
