...
int main(int argc, char *argv[]) {
  ...
  auto opt_size_m = op.addOption<std::size_t>(
      {'m', "size-m", "M - number of rows (default 8194).", 8194});
  ...
  // temporary files
  std::string gold_file = "/tmp/lift_shocStencil2D_gold_" + size_string;
  std::string grid_file = "/tmp/lift_shocStencil2D_grid_" + size_string;
  std::string weights_file = "/tmp/lift_shocStencil2D_weights_" + size_string;
  ...
  // run the harness
  run_harness(all_run, M, N, grid_file, gold_file, weights_file,
