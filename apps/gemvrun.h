...
#ifndef EXECUTOR_GEMVRUN_H
#define EXECUTOR_GEMVRUN_H

#include <vector>

#include "run.h"

template <typename T> struct GEMVRun : public Run {
  // input matrix size
  std::size_t size;
   ...
   */
  GEMVRun(const std::vector<std::string> &values, size_t size,
  ...
  void setup(cl::Context context) override {
    ...
    kernel.setArg((unsigned)extra_local_args.size() +
                      (unsigned)extra_args.size() + 6,
                  (int)size);
    kernel.setArg((unsigned)extra_local_args.size() +
                      (unsigned)extra_args.size() + 7,
                  (int)size);
  }
};

#endif // EXECUTOR_GEMVRUN_H
