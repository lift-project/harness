...
#ifndef EXECUTOR_STENCILNOPADRUN_H
#define EXECUTOR_STENCILNOPADRUN_H

#include <cstddef>

#include "run.h"

template <typename T> struct StencilNoPadRun : public Run {
  // input matrix size
  std::size_t size;
   ...
   */
  StencilNoPadRun(const std::vector<std::string> &values, size_t size,
...
#endif // EXECUTOR_STENCILNOPADRUN_H
