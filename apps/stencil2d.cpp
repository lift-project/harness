...
int main(int argc, char *argv[]) {
	...
	// temporary files
	std::string gold_file = "/tmp/apart_stencil_gold_" + std::to_string(N);
	std::string grid_file = "/tmp/apart_stencil_grid_" + std::to_string(N);
	...
	auto all_run =
	    Csv::init([&](const std::vector<std::string> &values) -> std::shared_ptr<Run> {
		    return (opt_double->get()
				? std::shared_ptr<Run>(new StencilRun<double>(values, N))
				: std::shared_ptr<Run>(new StencilRun<float>(values, N)));
	    });
