...
#include "convolutionSeparableY_harness.h"
#include "options.h"

int main(int argc, char *argv[]) {
  OptParser op("Harness for simple 2D convolution.");
  ...
  auto opt_size_m = op.addOption<std::size_t>(
      {'m', "size-m", "M - number of rows (default 1024).", 1024});
  auto opt_size_n = op.addOption<std::size_t>(
      {'n', "size-n", "N - number of columns (default 1024).", 1024});
