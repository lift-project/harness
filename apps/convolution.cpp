...
// [local includes]
#include "convolution_harness.h"
#include "convolutionrun.h"
#include "options.h"

int main(int argc, char *argv[]) {
	OptParser op("Harness for simple 1D convolution.");
	...
	auto opt_size =
	    op.addOption<std::size_t>({'s', "size", "Matrix size (default 1024).", 1024});
	auto opt_transpose = op.addOption<bool>(
	    {0, "transpose-in", "Transpose the input matrix before computation.", false});
	...
	// temporary files
	std::string gold_file = "/tmp/apart_convolution_gold_" + std::to_string(N);
	std::string grid_file = "/tmp/apart_convolution_grid_" + std::to_string(N);
	...
	auto all_run =
	    Csv::init([&](const std::vector<std::string> &values) -> std::shared_ptr<Run> {
		    return (opt_double->get()
				? std::shared_ptr<Run>(new ConvolutionRun<double>(values, N))
				: std::shared_ptr<Run>(new ConvolutionRun<float>(values, N)));
	    });
	...
	// run the harness
	run_harness(all_run, N, grid_file, gold_file, opt_force->get(), opt_threaded->get(),
		    opt_binary->get());
}
