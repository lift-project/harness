...
Run::Run(const std::vector<std::string> &values, std::size_t default_local_0,
	...
	// Local NDRange
	loc1 = Csv::readInt(
	    values[i++]); // returns 0 if it could not parse the string to an int, e.g. for '?'.
	loc2 = Csv::readInt(values[i++]);
	loc3 = Csv::readInt(values[i++]);

	if (loc1 == 0) loc1 = default_local_0;
	if (loc2 == 0) loc2 = default_local_1;
	if (loc3 == 0) loc3 = default_local_2;
