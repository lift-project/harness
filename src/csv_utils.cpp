...
init(std::function<std::shared_ptr<Run>(const std::vector<std::string> &)> factory) {
	using namespace std;

	// using exec_X.csv file
	...
	all_run.erase(remove_if(begin(all_run), end(all_run),
				[&](const shared_ptr<Run> &run) {
