...
TEST_F(GEMVTest, RunSquare) {
  ...
  auto settings = std::vector<std::string>(
      {"", size_string, "1", "1", "16", "1", "1", base_filename, "0", "0"});

  auto gemvRun = new GEMVRun<float>(settings, size, 0, 0, 0);
  auto run =
      std::vector<std::shared_ptr<::Run>>({std::shared_ptr<::Run>(gemvRun)});
  run_harness(run, size, "testFile1", "testFile2", "testFile3", "testFile4",
