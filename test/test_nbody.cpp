...
struct NBodyTest : public ::testing::Test {
	...
	virtual void SetUp() {
		if (File::is_file_exist(timing_filename)) std::remove(timing_filename.c_str());
	}
...
TEST_F(NBodyTest, CanCreate) {
	...
	auto settings =
	    std::vector<std::string>({"", "1024", "", "1", "16", "", "1", "bla", "0", "0"});
...
TEST_F(NBodyTest, Run) {
	...
	run_harness<float>(run, size, 500.0, 0.005, "testFile1", "testFile2", "testFile3", true,
			   false, false);
